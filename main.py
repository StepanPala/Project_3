...
def get_parsed_page(page_url) -> BeautifulSoup:
    ...
    Returns:
        BeautifulSoup: The parsed content of the page or None if an error occurred.
    """
    try:
...
def extract_table_data(parsed_soup: BeautifulSoup, i: int) -> tuple[list[str], list[str], list[str]]:
    """
    Extracts data from the main table for the given table index.

    Args:
        parsed_soup (BeautifulSoup): The parsed HTML content of the page.
        i (int): The index of the table to extract data from.

    Returns:
        list: A tuple containing three lists with the extracted data: codes, locations, and links.
    """
    # Extracts codes from the table using CSS selectors
    codes = [
        code_tag.text.strip()
        for code_tag in parsed_soup.select(f"td.cislo[headers='t{i}sa1 t{i}sb1']")
    ]
    # Creates an empty list to store the locations
    locations = []
    # Finds all td tags containing the location data
    location_tags = parsed_soup.find_all("td", headers=[f"t{i}sa1 t{i}sb2"])
...
def extract_detail_page_data(detail_soup: BeautifulSoup) -> tuple[str, str, str, dict[str, str]]:
    ...
    for j in range(1, 3):
        ...
        party_names = [
            ...
            for party_tag in detail_soup.select(f"td.overflow_name[headers='t{j}sa1 t{j}sb2']")
        ]
        # Extracts the party votes
        party_votes = [
            party_tag.text.strip()
            for party_tag in detail_soup.select(f"td.cislo[headers='t{j}sa2 t{j}sb3']")
        ]
...
def extract_data(parsed_soup: BeautifulSoup, detail_base_url: str, page_url: str) -> list[dict]:
    """
    ...
    for i in range(1, 4):
        codes, locations, links = extract_table_data(parsed_soup, i)

        # Iterates over the codes, locations, and links
        for code, location, link in zip(codes, locations, links):
            # Constructs the detail URL
            detail_url = detail_base_url + link

            # Parses the detail page
            detail_soup = get_parsed_page(detail_url)

            # If the detail page was successfully parsed
            if detail_soup:
                registered, envelopes, valid, party_results = extract_detail_page_data(detail_soup)
...
def get_command_line_arguments() -> tuple[str, str]:
    ...
    """
    if len(sys.argv) != 3:
        print("Invalid number of arguments. Please provide the URL and the output filename.")
        sys.exit(1)
